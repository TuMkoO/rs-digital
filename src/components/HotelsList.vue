<template>
  <div class="hotels mt-35">
    <ul class="list mb-40">
      <li
        v-for="(hotel, idx) in paginatedData"
        :key="idx"
        class="list-item border-grey rounded-15"
      >
        <div class="d-flex justify-content-between flex-column-sm">
          <div>
            <h2>{{ hotel.name }}</h2>
            <div
              class="info d-flex align-items-center flex-column-xsm align-items-start-xsm"
            >
              <span class="rating">
                <va-rating
                  v-model="hotel.stars"
              ...
              <span class="reviews-count"
                >&#8226; {{ hotel.reviews_amount }} отзыва</span
              >
              <span class="location"
          ...
          <div class="d-flex flex-column justify-content-between">
            <div class="price">
              {{ hotel.min_price }} ₽ <span>Цена за 1 ночь</span>
            </div>
            <!-- <button type="button" class="btn btn-sm btn-green-100">
              <span class="btn-icon"
                ><svg
                  width="14"
                  height="12"
            ...
            <button type="button" class="btn btn-sm btn-purple-100">
              <span class="btn-icon">
                <svg
                  width="15"
                  height="12"
    ...
    <div class="list-nav">
      <nav v-if="values.length > perPage || currentPage > 1">
        <ul class="pagination" v-if="pages.length">
          <li class="page-item">
            ...
            >
              <span class="page-link-control__prev">
                <svg
                  width="8"
                  height="12"
                  viewBox="0 0 8 12"
                ...
                >
                  <path
                    d="M6.95605 1L1.99953 5.95652L6.54301 10.5"
                    stroke="#3A3A3A"
                    stroke-width="2"
                  />
                </svg>
          ...
          <li class="page-item">
            ...
            >
              <span class="page-link-control__next">
                Следующая
                <svg
                  width="8"
                  height="12"
                ...
                >
                  <path
                    d="M0.999023 1L5.95555 5.95652L1.41207 10.5"
                    stroke="#3A3A3A"
                    stroke-width="2"
                  />
                </svg>
...
export default {
  props: ["values", "perPage"],
  setup(props) {
    //максимальное количество нумерных кнопок страниц
    ...
    //всего страниц
    const totalPages = ref(Math.ceil(props.values.length / props.perPage));
    ...
    const pages = computed(() => {
      let range = [];
      if (props.values.length && endPage.value) {
    ...
    watch(
      () => props.values,
      (item) => {
        if (item.length < 11 && currentPage.value > 1) {
          currentPage.value = 1;
        }
      }
    );
